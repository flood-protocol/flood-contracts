...
pub mod vm {
    ...
    pub struct RollForkWithForkIdCall {
    ...
    pub enum VmCalls {
        ...
        Roll(RollCall),
        RollForkWithForkId(RollForkWithForkIdCall),
    ...
    impl ethers::core::abi::AbiDecode for VmCalls {
        fn decode(
            ...
            if let Ok(decoded) =
                <RollForkWithForkIdCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(VmCalls::RollForkWithForkId(decoded));
            }
            if let Ok(decoded) =
    ...
    impl ethers::core::abi::AbiEncode for VmCalls {
        fn encode(self) -> Vec<u8> {
            match self {
                ...
                VmCalls::RollForkWithForkId(element) => element.encode(),
    ...
    impl ::std::fmt::Display for VmCalls {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                ...
                VmCalls::RollForkWithForkId(element) => element.fmt(f),
    ...
    impl ::std::convert::From<RollForkWithForkIdCall> for VmCalls {
        fn from(var: RollForkWithForkIdCall) -> Self {
            VmCalls::RollForkWithForkId(var)
        }
    }
