...
contract MaliciousFulfiller {
    ...
    function sourceConsideration(
        ...
        // send one wei less to try trick fulfiller
        IFloodPlain.Item calldata item = order.consideration;
        if (item.token == address(0)) payable(msg.sender).sendValue(item.amount - 1);
        else IERC20(item.token).safeIncreaseAllowance(msg.sender, item.amount - 1);
