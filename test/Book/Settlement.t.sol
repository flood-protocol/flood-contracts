...
import {Book,TradeStatus, Book__TradeNotFilled, Book__DisputePeriodNotOver} from "src/Book.sol";
...
contract SettlementTest is DisputeFixture {
    ...
    function testSettlement() public {
        ...
        (uint256 filledAtBlockInStorageAfter,address filledByInStorageAfter, TradeStatus statusInStorageAfter,bool unwrapOutputInStorageAfter, bool isEthTradeInStorageAfter, uint amountPaidAfter) = book.tradesData(id);
        ...
        assertEq(uint(statusInStorageAfter), uint(TradeStatus.UNINITIALIZED), "Status should be equal to UNINITIALIZED");
        assertFalse(unwrapOutputInStorageAfter, "Unwrap output should be false");
        assertFalse(isEthTradeInStorageAfter, "Is eth trade should be false");
        assertEq(amountPaidAfter, 0, "Amount paid should be equal to 0");
